...
trait PostMarshalSupport extends SprayJsonSupport with ScalaXmlSupport with DefaultJsonProtocol {
  ...
  def marshalPosts(posts: Iterable[PostAsm]): NodeSeq =
    <posts>{posts.map(marshalPost)}</posts>

  implicit def postsXmlFormat =
    Marshaller.opaque[Iterable[PostAsm], NodeSeq](marshalPosts)
  implicit def catalogItemXmlFormat =
    Marshaller.opaque[PostAsm, NodeSeq](marshalPost)

  def feedXml(feed: Future[Feed]): Future[NodeSeq] =
    feed.map(feed => <feed xmlns="http://www.w3.org/2005/Atom">
